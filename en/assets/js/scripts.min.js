...
const CONTACT_API_URL="https://mailapi.isymap.com/",RECAPTCHA_SITE_KEY="6Lek4CAsAAAAAMJmjs-21-JBevAZTk21Jy35f5aP";let isSubmitting=!1;async function getRecaptchaToken(){if("undefined"==typeof grecaptcha)return console.warn("reCAPTCHA non chargé - envoi sans token"),null;try{await new Promise(((e,t)=>{grecaptcha.ready((()=>e())),setTimeout((()=>t(new Error("reCAPTCHA timeout"))),5e3)}));return await grecaptcha.execute(RECAPTCHA_SITE_KEY,{action:"contact"})}catch(e){return console.warn("Erreur reCAPTCHA (cookies tiers bloqués ?):",e.message),null}}async function handleFormSubmit(e){if(e.preventDefault(),isSubmitting)return void console.log("Soumission déjà en cours...");const t=e.target,n=t.querySelector(".contact-from-envoyer"),o=n.textContent;isSubmitting=!0,n.disabled=!0,n.textContent="Envoi en cours...",n.classList.add("loading");try{const e=await getRecaptchaToken(),n={name:t.nom.value.trim(),email:t.email.value.trim(),subject:"Contact depuis le site ISYmap",message:constructMessage(t),recaptcha_token:e};if(!validateForm(n))return;const o=await fetch(`${CONTACT_API_URL}contact`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)}),s=await o.json();o.ok?(showSuccessMessage(),t.reset()):(400===o.status?showErrorMessage("Votre message n'a pas pu être envoyé. Veuillez vérifier le contenu et réessayer."):422===o.status?showErrorMessage("Certains champs ne sont pas correctement remplis. Veuillez vérifier vos informations."):showErrorMessage("Une erreur est survenue lors de l'envoi. Veuillez réessayer plus tard."),console.error("Erreur API:",s))}catch(e){console.error("Erreur lors de l'envoi:",e),showErrorMessage("Impossible de contacter le serveur. Veuillez vérifier votre connexion internet et réessayer.")}finally{isSubmitting=!1,n.disabled=!1,n.textContent=o,n.classList.remove("loading")}}function constructMessage(e){const t=e.nom.value.trim(),n=e.prenom.value.trim(),o=e.societe.value.trim();let s="";return(t||n)&&(s+=`Nom complet: ${t}${n?" "+n:""}\n`),o&&(s+=`Société: ${o}\n`),s+="\n--- Message ---\n\n",s+=e.message.value.trim(),s}function validateForm(e){return!e.name||e.name.length<2?(showErrorMessage("Le nom doit contenir au moins 2 caractères."),!1):e.email&&isValidEmail(e.email)?!(!e.message||e.message.length<10)||(showErrorMessage("Le message doit contenir au moins 10 caractères."),!1):(showErrorMessage("Veuillez entrer une adresse email valide."),!1)}function isValidEmail(e){return/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e)}function showSuccessMessage(){const e=document.getElementById("contact-form"),t=e.closest(".contact-form-container"),n=document.createElement("div");n.className="contact-success-message",n.innerHTML='\n        <div class="alert alert-success" role="alert">\n            <h4>Message envoye avec succes !</h4>\n            <p>Merci de nous avoir contactés. Nous vous répondrons dans les plus brefs délais.</p>\n        </div>\n    ',t.insertBefore(n,e),n.scrollIntoView({behavior:"smooth",block:"center"}),e.style.opacity="0.5",setTimeout((()=>{n.remove(),e.style.opacity="1"}),1e4)}function showErrorMessage(e){const t=document.getElementById("contact-form"),n=t.closest(".contact-form-container");n.querySelectorAll(".contact-error-message").forEach((e=>e.remove()));const o=document.createElement("div");o.className="contact-error-message",o.innerHTML=`\n        <div class="alert alert-danger" role="alert">\n            <h4>Erreur</h4>\n            <p>${e}</p>\n        </div>\n    `,n.insertBefore(o,t),o.scrollIntoView({behavior:"smooth",block:"center"}),setTimeout((()=>{o.remove()}),8e3)}async function checkAPIHealth(){try{return(await fetch(`${CONTACT_API_URL}/health`,{method:"GET",headers:{"Content-Type":"application/json"}})).ok?(console.log("API de contact disponible"),!0):(console.warn("API de contact ne répond pas correctement"),!1)}catch(e){return console.error("API de contact inaccessible:",e),!1}}document.addEventListener("DOMContentLoaded",(function(){const e=document.getElementById("contact-form");e&&(console.log("Formulaire de contact détecté - Initialisation..."),e.addEventListener("submit",handleFormSubmit))}));
